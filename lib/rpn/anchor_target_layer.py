...
class AnchorTargetLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        # labels
        top[0].reshape(ims_per_batch, 1, A * height, width)
        # bbox_targets
        top[1].reshape(ims_per_batch, A * 4, height, width)
        # bbox_inside_weights
        top[2].reshape(ims_per_batch, A * 4, height, width)
        # bbox_outside_weights
        top[3].reshape(ims_per_batch, A * 4, height, width)
    ...
    def forward(self, bottom, top):
        ...
        # map of shape (..., H, W)
        height, width = bottom[0].data.shape[-2:]
        num_imgs = bottom[0].data.shape[0]
        ...
        for im_i in range(num_imgs):
            ...
            inds_inside = np.where(
                ...
                (all_anchors[:, 2] < im_info[im_i][1] + self._allowed_border) &  # width
                (all_anchors[:, 3] < im_info[im_i][0] + self._allowed_border)  # height
            )[0]
            ...
            curr_im_inds = np.where(im_inds == im_i)[0];
            ...
            overlaps = bbox_overlaps(
                ...
                np.ascontiguousarray(curr_gt_boxes, dtype=np.float))
        ...
        # labels
        # print 'Copy check';
        labels_all = labels_all.reshape((num_imgs, height, width, A)).transpose(0, 3, 1, 2)
        labels_all = labels_all.reshape((num_imgs, 1, A * height, width))
        top[0].reshape(*labels_all.shape)
        top[0].data[...] = labels_all

        # bbox_targets
        bbox_targets_all = bbox_targets_all \
            .reshape((num_imgs, height, width, A * 4)).transpose(0, 3, 1, 2)
